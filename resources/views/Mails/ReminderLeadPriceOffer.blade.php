...
<head>
    ...
    <title>Reminder: Your Price Offer from Broom Service</title>
</head>
<body style="font-family: 'Open Sans', sans-serif; color: #212529; background: #fcfcfc;">
    <div style="max-width: 650px; margin: 0 auto; margin-top: 30px; margin-bottom: 20px; background: #fff; border: 1px solid #e6e8eb; border-radius: 6px; padding: 20px;">
        ...
        <h1 style="text-align: center;">{{ __('mail.price_offer_reminder.header') }}</h1>
        <p style="text-align: center;">{{ __('mail.price_offer_reminder.greeting', ['name' => $client['firstname']]) }}</p>
        <p style="text-align: center;">{{ __('mail.price_offer_reminder.content') }}</p>
        <p style="text-align: center;">{{ __('mail.price_offer_reminder.assistance') }}</p>
