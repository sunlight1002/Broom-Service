...
<head>
    ...
    <title>{{ __('mail.file_submission_request.subject') }}</title>
</head>
<body style="font-family: 'Open Sans', sans-serif; color: #212529; background: #fcfcfc;">
    <div style="max-width: 650px; margin: 0 auto; margin-top: 30px; margin-bottom: 20px; background: #fff; border: 1px solid #e6e8eb; border-radius: 6px; padding: 20px;">
        ...
        <h1 style="text-align: center;">{{ __('mail.file_submission_request.subject') }}</h1>
        <p style="text-align: center;">{{ __('mail.file_submission_request.greeting', ['name' => $client['firstname']]) }}</p>
        <p style="text-align: center;">{{ __('mail.file_submission_request.content') }}</p>
        <p style="text-align: center;">{{ __('mail.file_submission_request.request') }}</p>
        <p style="text-align: center;">
            {{ __('mail.file_submission_request.instructions') }}
        </p>
