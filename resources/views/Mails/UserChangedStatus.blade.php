...
<head>
    ...
    <title>{{ __('mail.client.status_changed.subject') }}</title>
</head>
<body style="font-family: 'Open Sans', sans-serif; color: #212529; background: #fcfcfc;">
    ...
    <div style="max-width: 650px; margin: 0 auto; margin-top: 30px; margin-bottom: 20px; background: #fff; border: 1px solid #e6e8eb; border-radius: 6px; padding: 20px;">
        ...
        <p style="text-align: center; line-height: 30px"> {{ __('mail.user_status_changed.header') }}</p>
        <p style="text-align: center; line-height: 30px"> {{ __('mail.user_status_changed.content', ['name' => $client['firstname'] . ' ' . $client['lastname'], 'status' => $status]) }}</p>
