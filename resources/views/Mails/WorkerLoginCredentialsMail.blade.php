...
<head>
	...
	<title>Worker Credentials</title>
</head>
@if($lng == 'heb')
...
@endif
	...
	<div style="max-width: 650px;margin: 0 auto;margin-top: 30px;margin-bottom: 20px;background: #fff;border: 1px solid #e6e8eb;border-radius: 6px;padding: 20px;">
		...
		<p style="text-align: center;line-height: 30px">{{__('mail.worker_credentials.content')}}</p>

		<p style="text-align: center;">{{__('mail.worker_credentials.email')}} :  {{ $email }} </p>
		<p style="text-align: center;">{{__('mail.worker_credentials.password')}} : {{ $passcode }}</p>
		<div style="text-align: center;">
			<a href='{{ url("worker/login")}}' style="background: #ef6c6b;color: #fff;border: 1px solid #ef6c6b;font-size: 16px;padding: 8px 20px;border-radius: 8px;cursor: pointer;text-decoration: none;text-align: center;">{{__('mail.worker_credentials.btn_txt')}}</a> 
		</div>
