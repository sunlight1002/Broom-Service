...
<head>
	...
	<title>Safety and Gear Form Signed</title>
</head>

<body @if($worker->lng=='heb' ) style="font-family: 'Noto Sans Hebrew', sans-serif;color: #212529;background: #fcfcfc; direction:rtl" @else style="font-family: 'Open Sans', sans-serif;color: #212529;background: #fcfcfc;" @endif>
	<div style="max-width: 650px;margin: 0 auto;margin-top: 30px;margin-bottom: 20px;background: #fff;border: 1px solid #e6e8eb;border-radius: 6px;padding: 20px;">
		...
		<h1 style="text-align: center;">{{__('mail.common.salutation', ['name' => $worker->firstname . ' ' . $worker->lastname])}}</h1>

		<p style="text-align: center;line-height: 30px">{{__('mail.common.greetings')}}</p>
		<p style="text-align: center;line-height: 30px">{{__('mail.worker.safety-and-gear-signed.message')}}</p>
