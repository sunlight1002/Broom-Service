...
<body @if($client['lng']=='heb' ) style="font-family: 'Noto Sans Hebrew', sans-serif;color: #212529;background: #fcfcfc; direction:rtl" @else style="font-family: 'Open Sans', sans-serif;color: #212529;background: #fcfcfc;" @endif>
	<div style="max-width: 650px;margin: 0 auto;margin-top: 30px;margin-bottom: 20px;background: #fff;border: 1px solid #e6e8eb;border-radius: 6px;padding: 20px;">
		...
		<p style="text-align: center;">{{__('mail.meeting.greetings')}} {{__('mail.meeting.from')}} {{__('mail.meeting.company')}}. {{__('mail.meeting.appointment')}}
			...
			{{__('mail.meeting.address_txt')}} <span style="color:#0130c6;font-weight:700;">{{ isset($property_address)?$property_address['address_name']:'NA' }}</span> {{__('mail.meeting.select_preferred_slot')}}
			@endif
			...
			<span style="color:#0130c6;font-weight:700;">{{ __('mail.meeting.price_offer') }}&nbsp;</span>
			@elseif($purpose == "Quality check")
			<span style="color:#0130c6;font-weight:700;">{{ __('mail.meeting.quality_check') }}&nbsp;</span>
			@else
			<span style="color:#0130c6;font-weight:700;">{{ $purpose }}&nbsp;</span>
			@endif
		</p>
		...
		<div style="display:flex;justify-content: center">
			...
			{{--<a href='{{ url("meetings/".base64_encode($id)."/choose-slot")}}' target='_blank' style="background: #ADD8E6;color: #fff;border: 1px solid #ADD8E6;font-size: 16px;padding: 10px 24px;border-radius: 4px;cursor: pointer;text-decoration: none;text-align: center">{{__('mail.meeting.choose_slot')}}</a>--}}
		</div>
