...
<head>
    ...
    <title>Lead Declined Price Offer | Broom Service</title>
</head>
<body style="font-family: 'Open Sans', sans-serif; color: #212529; background: #fcfcfc;">
    <div style="max-width: 650px; margin: 0 auto; margin-top: 30px; margin-bottom: 20px; background: #fff; border: 1px solid #e6e8eb; border-radius: 6px; padding: 20px;">
        ...
        <h1 style="text-align: center;">{{ __('mail.price_offer_decline.header') }}</h1>
        <p style="text-align: center;">{{ __('mail.price_offer_decline.greeting', ['name' => $client['firstname']]) }}</p>
        <p style="text-align: center;">{{ __('mail.price_offer_decline.content') }}</p>
        <p style="text-align: center;">
            {{ __('mail.price_offer_decline.details', [
                'client_name' => $client['firstname'] . ' ' . $client['lastname'],
                'reason' => $client['decline_reason'] ?? __('mail.price_offer_decline.no_reason_provided')
            ]) }}
        </p>
        <p style="text-align: center;">{{ __('mail.price_offer_decline.assistance') }}</p>
