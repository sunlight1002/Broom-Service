...
<head>
    ...
    <title>Follow-Up Required: Pending Lead | Broom Service</title>
</head>
<body style="font-family: 'Open Sans', sans-serif; color: #212529; background: #fcfcfc;">
    <div style="max-width: 650px; margin: 0 auto; margin-top: 30px; margin-bottom: 20px; background: #fff; border: 1px solid #e6e8eb; border-radius: 6px; padding: 20px;">
        ...
        <h1 style="text-align: center;">{{ __('mail.pending_lead.header') }}</h1>
        <p style="text-align: center;">{{ __('mail.pending_lead.greeting') }}</p>
        <p style="text-align: center;">{{ __('mail.pending_lead.reminder') }}</p>
        <p style="text-align: center;">
            <strong>{{ __('mail.pending_lead.lead_name') }}:</strong> {{ $client['firstname'] }} {{ $client['lastname'] }}<br>
            <strong>{{ __('mail.pending_lead.contact_info') }}:</strong> {{ $client['phone'] }} / {{ $client['email'] }}<br>
            <strong>{{ __('mail.pending_lead.inquiry_date') }}:</strong> {{ \Carbon\Carbon::parse($client['created_at'])->format('d-m-Y') }}
        </p>
