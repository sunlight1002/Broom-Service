...
<body dir="{{ $client['lng'] == 'heb' ? 'rtl' : 'ltr' }}" style="direction: {{ $client['lng'] == 'heb' ? 'rtl' : 'ltr' }}; text-align: {{ $client['lng'] == 'heb' ? 'right' : 'left' }}; font-family: {{ $client['lng'] == 'heb' ? '\'Noto Sans Hebrew\', sans-serif' : '\'Open Sans\', sans-serif' }};">
    ...
    <div class="container">
        ...
        <h1 class="center">
            {{ __('mail.common.salutation', ['name' => $client['firstname'] . ' ' . $client['lastname']]) }}</h1>
        ...
        <p class="center">{{ __('mail.meeting.content_for_off_site') }} </p>
        ...
        <div class="center">
            <a href='{{ url('meeting-files/' . base64_encode($id)) }}' style="margin-top: 5px; color: white;"
                target='_blank' class="action-button black">{{ __('mail.meeting.upload_job_description') }}</a>
        </div>
