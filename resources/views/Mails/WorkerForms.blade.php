...
<head>
	...
	<title>{{__('mail.forms.worker_forms')}}</title>
</head>
@if($lng == 'heb')
...
@endif
	...
	<div style="max-width: 650px;margin: 0 auto;margin-top: 30px;margin-bottom: 20px;background: #fff;border: 1px solid #e6e8eb;border-radius: 6px;padding: 20px;">
		...
		<div style="text-align: center;">
			<a href='{{ url("worker-forms/".base64_encode($id))}}' style="background: #ef6c6b;color: #fff;border: 1px solid #ef6c6b;font-size: 16px;padding: 10px 24px;border-radius: 4px;cursor: pointer;text-decoration: none;margin-top: 25px;margin-bottom: 25px">{{__('mail.forms.btn_txt')}}</a> 
		</div>
