...
export default function Invoices() {
    ...
    const handlePageClick = async (data) => {
        ...
        axios
            .get("/api/admin/invoices?page=" + currentPage + "&" + filtered, {
                headers,
            })
            .then((response) => {
    ...
    const filter = (e) => {
        ...
        setFiltered(d);
        axios.get(`/api/admin/invoices?${d}`, { headers }).then((res) => {
            if (res.data.invoices.data.length > 0) {
                setInvoices(res.data.invoices.data);
                setPageCount(res.data.invoices.last_page);
            } else {
    ...
    const dfilter = (fil) => {
        ...
        axios.get(`/api/admin/invoices?${fil}`, { headers }).then((res) => {
            if (res.data.invoices.data.length > 0) {
                setInvoices(res.data.invoices.data);
                setPageCount(res.data.invoices.last_page);
            } else {
        ...
        <div id="container">
            ...
            >
                <div className="modal-dialog" role="document">
                    <div className="modal-content">
                        <div className="modal-header">
                            <h5 className="modal-title" id="exampleModalCancel">
                                Cancel Reason
                            </h5>
                            <button
                                type="button"
