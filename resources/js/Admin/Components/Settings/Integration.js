...
export default function Integration() {
  ...
  const checkGoogleCalendarStatus = async () => {
    try {
      const response = await axios.get('/api/admin/google/auth', { headers });
      setIsGoogleConnected(response.data.action === 'connected');
    } catch (error) {
      console.error('Error:', error);
      alert.error('An error occurred while checking Google Calendar connection.');
    }
  };
  ...
  const handleGoogleCalendarClick = async () => {
    try {
      ...
      console.error('Error:', error);
      alert.error('An error occurred while trying to connect to Google Calendar.');
    }
  };
  ...
  const handleRemoveGoogleCalendar = async () => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          const response = await axios.delete('/api/admin/google/disconnect', { headers });
          setIsLoading(false);
          ...
          if (response.status === 200) {
            ...
            Swal.fire(
              "Disconnected!",
              response.data.message || "Google Calendar has been disconnected.",
              "success"
            );
          } else {
          ...
          Swal.fire({
            title: "Error!",
            text: error.response?.data?.message || "An error occurred while disconnecting Google Calendar.",
            icon: "error",
          });
        }
      }
      ...
      {role && role === "superadmin" && (
        <>
          ...
          {isGoogleConnected ? (
            <>
              <div className="form-group mt-4">
                <label htmlFor="calendarSelect">Select Calendar:</label>
            ...
            <p className="mt-4 text-center">Please connect your google account to schedule a meeting.</p>
          )}
        </>
      )}
