...
export default function WorkerPendingRequest({ workerId }) {
    ...
    const handleChangeStatus = async (userId, e) => {
        ...
        try {
            const response = await axios.put(
                `/api/admin/schedule-changes/${userId}`,
    ...
    const getRequest = async (id) => {
        const response = await axios.get(`/api/admin/schedule-change/${id}`, {
            headers,
        });
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const response = await axios.post(
                `/api/admin/send-message-to-user/${userId}`,
                data,
                { headers }
            );
            setIsOpen(false);
    ...
    const initializeDataTable = (initialPage = 0) => {
        if (!$.fn.DataTable.isDataTable(tableRef.current)) {
            ...
            const stateConfig = getDataTableStateConfig(TABLE_IDS.WORKER_PENDING_REQUESTS, {
                onStateLoad: (settings, data) => {
                    console.log('Worker pending requests table state loaded:', data);
                },
                onStateSave: (settings, data) => {
                    console.log('Worker pending requests table state saved:', data);
                }
            });
