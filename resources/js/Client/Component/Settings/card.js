...
export default function card() {
    ...
    const handleCard = (e) => {
        ...
        if (!ncard) {
            window.alert(t("work-contract.messages.card_msg"));
            return false;
        }
        if (ncard.length < 16) {
            window.alert(t("work-contract.messages.card_msg2"));
            return false;
        }
        if (exy == "0") {
            window.alert(t("work-contract.messages.card_year"));
            return false;
        }
        if (exm == "0") {
            window.alert(t("work-contract.messages.card_month"));
            return false;
        }
        if (!cvv) {
        ...
        if (cvv.length < 3) {
            window.alert(t("work-contract.messages.invalid_cvv"));
            return false;
        }
        ...
        axios(config).then(function (response) {
            ...
            axios(txnConfig).then(function (res) {
                ...
                axios
                    ...
                    .then((re) => {
                        ...
                        swal(
                            t("work-contract.messages.card_success_panel"),
                            "",
                            "success"
                        );
                        getCard();
                                ...
                                <div className="col-sm-12">
                                    <div className="form-group">
                                        ...
                                        <select
                                            ...
                                            className="form-control"
                                            onChange={(e) =>
                                                setExy(e.target.value)
                                            }
                                        >
                                            <option value="0">
                                                {t("work-contract.select_year")}
                                            </option>
                                        </select>
                                    </div>
                                </div>
