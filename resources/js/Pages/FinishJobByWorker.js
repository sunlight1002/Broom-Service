...
import ClientDetails from "../Worker/Components/Job/ClientDetails";
...
export default function FinishJobByWorker() {
    ...
    const getJob = () => {
        axios
            .get(`/api/get-job-by-uuid/${params.uuid}`)
            .then((res) => {
    ...
    const handleApproveJob = (e) => {
        ...
        axios
            .post(
                `/api/guest/${worker.id}/jobs/${job.id}/approve`
            )
            .then((res) => {
                getJob();
    ...
    const startTimer = () => {
        ...
        setIsRunning(true);
        axios
            .post(`/api/jobs/start-time-uuid/${params.uuid}`)
            .then((res) => {
                getTime();
                setTimeout(() => {
    ...
    const stopTimer = () => {
        ...
        axios
            .post(`/api/jobs/end-time-uuid/${params.uuid}`)
            .then((res) => {
                getTimes();
                setTimeout(() => {
                    setIsSubmitting(false);
    ...
    const getTime = () => {
        ...
        axios.post(`/api/get-job-time-uuid`, data).then((res) => {
            let t = res.data.time;
            if (t) {
    ...
    const getComments = () => {
        axios
            .get(`/api/jobs/${params.uuid}/job-comments-uuid`, {
                params: {
    ...
    const handleGetSkippedComments = async () => {
        try {
            const response = await axios.get(`/api/job-comments/skipped-job-comments/${commentId}`);
            console.log(response, "dwd");
    ...
    return (
        <div id="container">
            <div id="content" className="p-0 mx-5">
