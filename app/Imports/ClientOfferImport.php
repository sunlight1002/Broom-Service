        ...
        foreach ($collection as $row) {
            ...
            try {
                $validator = Validator::make($row->toArray(), [
                    'clients_primary_email' => ['required', 'string','email', 'max:255'],
                ]);
                ...
                $client = Client::Where('email', $row['clients_primary_email'])->first();
                $clientpropertyaddress = ClientPropertyAddress::Where('client_id', $client->id)->first();

                if (empty($client->id) || empty($clientpropertyaddress->id)) {
                    $failedImports->push($row);
                    continue;
                }
                ...
                if($row['type'] == 'hourly')
                {
                    $total_amount = $row['job_hours'] * $row['price'];   
                }
                ...
                $services = [
                    ...
                    'rateperhour' => ($row['type'] == 'hourly')? $row['price'] : '',
                    'freq_name' => $row['frequency'] ?? '',
                    'frequency' => $serviceschedule->id ?? '',
                    'fixed_price' => ($row['type'] == 'fixed')? $row['price'] : '',
                    ...
                    foreach ($existing_services as $existing_service) {
                        if (is_array($existing_service)) {
                            if (isset($existing_service['type']) && $existing_service['type'] == 'hourly') {
                                if (isset($existing_service['jobHours']) && is_numeric($existing_service['jobHours']) &&
