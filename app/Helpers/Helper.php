...
if (!function_exists('sendWorkerLeadWhatsappMessage')) {
    ...
    {
        \Log::info("sendWorkerLeadWhatsappMessage");
...
if (!function_exists('sendWorkerLeadWhatsappMediaMessage')) {
    function sendWorkerLeadWhatsappMediaMessage(
        $number,
        $mediaPath,
        $caption = '',
        $lang = 'he',
        ...
        try {
            ...
            // Upload the file as binary using withBody() and correct content type
            $response = Http::withHeaders([
                'Authorization' => 'Bearer ' . $numberToken,
                'accept' => 'application/json',
                'Content-Type' => $fileMimeType,
            ])->withBody(file_get_contents($mediaPath), $fileMimeType)
                ->post(config('services.whapi.url') . 'media');
...
if (!function_exists('sendWorkerLeadWhatsappImageMessage')) {
    function sendWorkerLeadWhatsappImageMessage(
        $number,
        $fullMediaPath,
        $caption = '',
        $mimeType = 'image/jpeg', // Adjust based on the image type (e.g., 'image/png')
