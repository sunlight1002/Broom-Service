...
class ClaimController extends Controller
{
    ...
    public function getWorkerClaim(Request $request)
    {
        $workerId = $request->query('worker_id');

        if (!$workerId) {
            return response()->json(['error' => 'Worker ID is required'], 400);
        }

        $claim = Claim::where('worker_id', $workerId)->latest()->first();

        if (!$claim) {
            return response()->json(['claim_description' => null], 200);
        }
