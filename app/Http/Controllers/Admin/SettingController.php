...
class SettingController extends Controller
{
    ...
    {
            $validator = Validator::make($request->all(), [
                'overtime_rate_9to10_hours' => 'required',
                'overtime_rate_11to12_hours' => 'required',
                'holiday_rate_forTwo_hours' => 'required',
                'holiday_rate_three_hours' => 'required',
                'rosh_hashanah_pay' => 'required',
                'public_sector_holiday_pay' =>'required',
                'bonus_after_one_year' =>'required',
                'bonus_after_six_years' =>'required',
            ]);
            ...
            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::OVERTIME_RATE_9TO10_HOURS ],
                ['value' => $request->overtime_rate_9to10_hours]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::OVERTIME_RATE_11TO12_HOURS ],
                ['value' => $request->overtime_rate_11to12_hours]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::HOLIDAY_RATE_FORTWO_HOURS ],
                ['value' => $request->holiday_rate_forTwo_hours]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::HOLIDAY_RATE_THREE_HOURS ],
                ['value' => $request->holiday_rate_three_hours]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::ROSH_HASHANAH_PAY ],
                ['value' => $request->rosh_hashanah_pay]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::PUBLIC_SECTOR_HOLIDAY_PAY ],
                ['value' => $request->public_sector_holiday_pay]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::BONUS_AFTER_ONE_YEAR ],
                ['value' => $request->bonus_after_one_year_perHour]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::BONUS_AFTER_SIX_YEARS ],
                ['value' => $request->bonus_after_six_years_perHour]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::DEDUCTION_FOREIGNWORKER ],
                ['value' =>$request->deduction_foreignworker]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::RECOVERY_FEE ],
                ['value' =>$request->recovery_fee_year_ofservice]
            );


            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::DRIVING_FEE_PERDAY ],
                ['value' =>$request->driving_fee_perDay]
            );

            Setting::updateOrCreate(
                ['key' => SettingKeyEnum::DRIVING_FEE_PERMONTH ],
                ['value' =>$request->driving_fee_perMonth]
            );
