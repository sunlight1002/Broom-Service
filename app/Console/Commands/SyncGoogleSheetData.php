...
class SyncGoogleSheetData extends Command
{
    ...
    {
        // $filePath = storage_path('crm.xlsx'); 
        ...
        try {
            $rows = $this->getAllGoogleSheetData();
            $sheetName = '';
    ...
    public function getAllGoogleSheetData()
    {
        // Retrieve the Google Sheet ID from settings
        $spreadsheetId = Setting::query()
            ->where('key', SettingKeyEnum::GOOGLE_SHEET_ID)
            ->value('value');
    
        // Retrieve the Google Access Token from settings
        ...
        try {
            ...
            $metadataHttpCode = $metadataResponse->status();
            $metadata = $metadataResponse->json();

            if ($metadataHttpCode == 401 || $metadataHttpCode == 403) {
            ...
            $sheetNames = array_map(fn($sheet) => $sheet['properties']['title'], $sheets);
            ...
            foreach ($sheetNames as $sheetName) {
                ...
                if ($httpCode != 200) {
                    Log::error("Failed to fetch sheet values for sheet: $sheetName", [
                        'http_code' => $httpCode,
                        'response' => $data
                    ]);
                    continue;
